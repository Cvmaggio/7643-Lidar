...
class Decoder(nn.Module):
    ...
    def __init__(self, emb_size, encoder_hidden_size, decoder_hidden_size, output_size, dropout=0.2, model_type="RNN"):
        ...
        self.recurrent = (nn.RNN if self.model_type == 'RNN' else nn.LSTM)(
    ...
    def forward(self, input, hidden):
        ...
        # make args
        args = (hidden, torch.zeros((1, input.shape[0], self.decoder_hidden_size)).to(hidden.device)) if self.model_type == 'LSTM' else (hidden)
        ...
        # drop extra tokens
        # output = output[:,0,:] if seq_len > 1 else output.reshape((-1, self.decoder_hidden_size))
        output = output[:,0,:] if seq_len > 1 else output
